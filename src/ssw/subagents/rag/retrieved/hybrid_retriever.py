...
from ssw.config import RRF_K
from ssw.core.logging import get_logger
from ssw.db.session import AsyncSessionLocal
from ssw.subagents.rag.retrieved.keyword_retriever import KeywordRetriever
from ssw.subagents.rag.retrieved.vector_retriever import RetrievedChunk, VectorRetriever
...
class HybridRetriever:
    ...
    async def search(
        ...
        vector_hits, keyword_hits = await asyncio.gather(
            self._safe_search(
                VectorRetriever, query, recall_top_k, "vector"
            ),
            self._safe_search(
                KeywordRetriever, query, recall_top_k, "keyword"
            ),
        )
        return rrf_fuse(
