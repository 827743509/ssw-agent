...
from ssw.subagents.rag.prompts import build_agent_plan_messages
from ssw.subagents.rag.query_rewriter import QueryRoute
...
class AgentPlanner:
    ...
    async def plan(
        ...
        try:
            response = await build_llm().ainvoke(messages)
