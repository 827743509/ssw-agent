...
from ssw.config import RERANK_TIMEOUT, RERANK_API_KEY, RERANK_MODEL, RERANK_BASE_URL
from ssw.core.exceptions import ConfigurationError
from ssw.core.logging import get_logger
from ssw.subagents.rag.retrieved.vector_retriever import RetrievedChunk
...
class Reranker:
    ...
    def _get_client(self) -> httpx.AsyncClient:
        if self._client is None:
            self._client = httpx.AsyncClient(timeout=RERANK_TIMEOUT)
        return self._client
    ...
    async def _fetch_scores(
        ...
        response = await client.post(
            RERANK_BASE_URL, json=payload, headers=headers
        )
