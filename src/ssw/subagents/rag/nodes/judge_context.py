...
from ssw.config import RERANK_MIN_SCORE, RETRIEVAL_MIN_SCORE
...
async def judge_context(state: RAGState) -> RAGState:
    ...
    if top.rerank_score is not None:
        is_enough = top.rerank_score >= RERANK_MIN_SCORE
    elif top.vector_score is not None:
        is_enough = top.vector_score >= RETRIEVAL_MIN_SCORE
    else:
        is_enough = False
    ...
    return {"context_is_enough": is_enough,"messages":[
        AIMessage(content=f"""
        知识库检索结果:
        {context}
        """)
