...
from deepagents import CompiledSubAgent, create_deep_agent, register_harness_profile, HarnessProfile
...
- 默认目标数据库是 MySQL。
- 必须优先阅读 Agent skills 目录中的表结构文档。
- 查询不到表结构时，回复未找到相关项目表结构。
...
text_to_sql_subagent: CompiledSubAgent = {
    "name": "text-to-sql",
    "description": (
        "将自然语言问题转换为 SQL 查询语句。适合需要从子 Agent skills 读取表结构、"
